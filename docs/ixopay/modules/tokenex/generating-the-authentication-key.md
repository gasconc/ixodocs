...
Detokenization is stricter: a [Detokenize Authentication Key](https://documentation.ixopay.com/modules/docs/tokenex/detokenize) is only valid with a timestamp less than 20 seconds old by default, configurable up to 60 seconds via `expiresInSeconds`. It is also only valid for one operation: data or cvv. In combined data/cvv mode, one key covers both operations. Re-using the same key for another operation returns the error `Invalid Authentication Key - Authentication key already used`.
info
...
Detokenize keys only: `timestamp` is second-resolution, so two keys for the same tokenExID/origin/token generated within the same second are identical, and the second use is rejected as already used  | 20180109161437 (January 9th, 2018 4:14:37 PM UTC, formatted in yyyyMMddHHmmss format)  |  
