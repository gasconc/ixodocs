...
tags:
- https-secure-ixopay-com-api-transaction-apikey-capture
- request-https-documentation-ixopay-com-api-pci-capture-request-direct-link-request
- path-parameters
- bodyrequired
- api
- json
...
```
POST 
## https://secure.ixopay.com/api/v3/transaction/:apiKey/capture

```A capture transaction completes the payment which was previously authorized with the [Preauthorize](https://documentation.ixopay.com/api/pci/preauthorize) method.
Depending on the payment method you can even capture only a partial amount of the authorized amount.
## Request[​](https://documentation.ixopay.com/api/pci/capture#request "Direct link to request")
### Path Parameters
...
### Body**required**
Data which is required to process a capture. Depending on the adapter, additional fields may be required to be sent as `extraData` and field restrictions may be stricter.
...
This flag indicates that this capture operation represents the final step, whether it concludes a series of captures or is a standalone capture. It is supported by specific adapters, contingent on the PSP's compatibility with this feature.
If an adapter lacks support for this functionality, no error will be raised, and the flag will be ignored. Importantly, activating this flag doesn't automatically capture any remaining amount.
**l2l3Data** object
...
```## Responses[​](https://documentation.ixopay.com/api/pci/capture#responses "Direct link to Responses")
  * 200
  * 400
  * 422
...
````"returnType": "FINISHED"` indicates a successfully completed capture transaction.
```
...
```  * curl
  ...
  * CURL
```
curl -L 'https://secure.ixopay.com/api/v3/transaction/:apiKey/capture' \  
