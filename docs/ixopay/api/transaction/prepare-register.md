...
```Prepare a register transaction, will not create a transaction on IXOPAY. Used with very specific payment adapters to prepare a frontend payment widget.
...
### Body
Data which is required to process a prepare register. Depending on the adapter, additional fields may be required to be sent as `extraData` and field restrictions may be stricter.
